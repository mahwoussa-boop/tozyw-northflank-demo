...
def _close_db_conn():
    """يغلق الاتصال الدائم عند إنهاء البرنامج (ResourceWarning)."""
    global _db_conn
    with _db_lock:
...
import atexit
atexit.register(_close_db_conn)
...
@_functools.lru_cache(maxsize=10000)
...
def _price(row):
    keys = row.index if hasattr(row, "index") else row  # dict أو Series
...
def _pid(row, id_col):
    ...
    keys = row.index if hasattr(row, "index") else row  # dict أو Series
    if not id_col or id_col not in keys:
        return ""
    v = row.get(id_col, "")
...
# ═══════════════════════════════════════════════════════
#  v36: كشف «مجموعة/طقم» متّسق مع normalize (طرفان متناظران)
#  normalize يحوّل «مجموعة»→«مgوعه»، فكشفها على النص المطبَّع بكلمات خام يفشل
#  على جهتنا (المطبَّعة) بينما ينجح على المنافس (الخام) ⇒ رفض مجموعة↔مجموعة خطأً.
#  الحل: نطبّع الكلمات المفتاحية مرة واحدة ونقارنها على النص المطبَّع للطرفين.
# ═══════════════════════════════════════════════════════
# v37: مواءمة مفردات الكشف مع classify_product (سطر ~1246). كانت coffret/gift/سيت/هدية
#      تُصنَّف «set» في classify لكن _is_set_norm يفوّتها ⇒ رفض مجموعة↔مجموعة شرعي حين
#      يستعمل أحد الطرفين مفردةً من هنا والآخر من هناك. نتجنّب «set» الإنجليزية المجرّدة
#      عمداً: مطابقة substring على النص المطبَّع تُخطئ في sunset/closet (classify يحرسها بمسافة).
_SET_KW_NORM = tuple(dict.fromkeys(
    normalize(w).lower() for w in ('مجموعة', 'مجموعه', 'طقم', 'gift set', 'gift box',
                                   'coffret', 'gift', 'هدية', 'سيت')
))


def _is_set_norm(norm_text: str) -> bool:
    """True إذا كان النص (المطبَّع مسبقاً) مجموعة/طقم — كشف صامد أمام normalize."""
    if not norm_text:
        return False
    nl = norm_text.lower()
    return any(k in nl for k in _SET_KW_NORM)


# v31.11b: حارس الكلمات المميزة الإلزامية — ثابت لا يتغيّر أبداً بين عمليات
# البحث، لذا يُبنى مرة واحدة على مستوى الوحدة بدل إعادة إنشائه كقاموس حرفي
# جديد لكل مرشّح داخل CompIndex.search() (حتى 30 مرشحاً × كل استدعاء بحث).
...
class CompIndex:
    ...
    def __init__(self, df, name_col, id_col, comp_name, img_col=None, url_col=None, gallery_col=None):
        self.comp_name = comp_name
        self.name_col  = name_col
        self.id_col    = id_col
        self.img_col   = (img_col or "") or ""
        self.url_col   = (url_col or "") or ""
        self.gallery_col = (gallery_col or "") or ""
        ...
        if _price_cols:
            ...
            self.prices = [_price_from_dict(row) for row in self.df.to_dict('records')]
        ...
        if self.url_col and self.url_col in self.df.columns:
            ...
            self.extra_urls = [""] * n
        # ج3 خ2: معرض صور بطاقة المقارنة — عرض فقط، لا يمسّ المطابقة/الدرجة.
        if self.gallery_col and self.gallery_col in self.df.columns:
            from utils.data_helpers import all_image_urls_list
            self.extra_gallery = [
                all_image_urls_list(v) for v in self.df[self.gallery_col].fillna("").astype(str)
            ]
        else:
            self.extra_gallery = [[]] * n
    ...
    def search(self, our_norm, our_br, our_sz, our_tp, our_gd, our_pline="", top_n=6, our_price=0):
        ...
        # v36: كشف مجموعة ثابت للبحث كله — normalize يشوّه «مجموعة» فيُخطئ classify
        #      ويُرجع 'retail' لمجموعاتنا؛ نثبّتها 'set' لتناظر فلتر التصنيف مع المنافس.
        _our_is_set = _is_set_norm(o_n_low)
        if _our_is_set:
            our_class = 'set'
        # ⚡ o_n_low ثابت طوال هذا البحث — عضوية Flanker لمنتجنا تُحسَب مرة واحدة
        # هنا بدل إعادة حسابها لكل مرشّح داخل الحلقة أدناه (كانت حتى 30× مكرَّرة).
        _our_flanker_membership = [
            _group_in_text(gi, o_n_low) for gi in range(len(_FLANKER_GROUPS))
        ]
        ...
        for _, fast_score, vi in fast:
            ...
            if our_class != c_class:
                ...
                if (our_class == 'tester') != (c_class == 'tester'):
                    continue
            # v36: حارس المجموعات/الطقم — لا نطابق مجموعة مع عطر فردي أبداً.
            #      مقارنة متناظرة على النص المطبَّع للطرفين (كانت جهتنا مطبَّعة
            #      والمنافس خام ⇒ رفض مجموعة↔مجموعة الشرعي). set↔set يمرّ الآن.
            _comp_is_set = _is_set_norm(self.norm_names[idx] if idx < len(self.norm_names) else "")
            ...
            # v35: نقارن مطبَّعاً↔مطبَّعاً — o_n_low مطبَّع أصلاً بينما الاسم الخام
            #      يحمل همزات تُفشل الكشف («اورا»⊂«اوراق» عندنا مقابل «أوراق»
            #      الخام عند المنافس ⇒ رفض التوأم المطابق 100%)
            c_flank_low = self.norm_names[idx].lower() if idx < len(self.norm_names) else c_n_low
            _flanker_mismatch = False
            for gi in range(len(_FLANKER_GROUPS)):
                in_comp = _group_in_text(gi, c_flank_low)
                if _our_flanker_membership[gi] != in_comp:
            ...
            if our_br and c_br:
                # _MUST_MATCH_WORDS ثابت وحدة (سطر ~1768) — لا يُعاد بناؤه هنا.
                _dist_reject = False
            ...
            gallery_u = self.extra_gallery[idx] if idx < len(self.extra_gallery) else []
            cands.append({
                ...
                "thumb": img_u, "comp_images": gallery_u,
            })
...
def _our_brand_candidates(extracted: str, brand_from_row: str) -> list:
    """v35: مفاتيح ماركة مرشّحة للبحث ماركة-أولاً — تُجرَّب كلها بلا تكرار.

    اسم منتجنا قد لا يحمل الماركة، وعمود سلة «عربي | English» قد لا يكون في
    KNOWN_BRANDS (روسيندو ماتيو…)، والمنافسون يخزّنون الماركة بصيغ متعددة
    تنتج مفاتيح فهرسة متنافرة (مزدوجة/عربية/إنجليزية) — فنجرّب بالترتيب:
    المستخرَجة ثم extract_brand(العمود) ثم العمود كاملاً ثم كل شطر حول «|».
    التكرار يُزال بمفتاح الفهرسة النهائي normalize().lower() نفسه، فلا يُعاد
    البحث الضبابي على نفس دلو الماركة مرتين.
    """
    cands: list = []
    seen: set = set()
    ...
    _add(extracted)
    bfr = (brand_from_row or "").strip()
    if bfr:
        _add(extract_brand(bfr))
        _add(bfr)
        for part in bfr.split("|"):
            _add(part)
    return cands


def _brand_for_pline(product: str, extracted: str, brand_keys: list) -> str:
    """v35: ماركة تجريد خطّ الإنتاج — عند غياب المستخرَجة نلتقط أول مفتاح
    يظهر فعلاً في الاسم (extract_product_line تجرّد بالاستبدال النصّي فقط،
    وإلا بقيت الماركة داخل خطّنا وسقط المرشح على بوّابة PLINE<60)."""
    if extracted:
        return extracted
    pl = (product or "").lower()
    pn = normalize(product or "")
    for k in brand_keys:
        kl = (k or "").lower()
        kn = normalize(k or "")
        if (kl and kl in pl) or (kn and kn in pn):
            return k
    return ""


def _search_indices(indices: dict, our_n: str, brand_keys: list, size, ptype,
                    gender, our_pline: str = "", top_n: int = 6, our_price=0) -> list:
    """v35: يجمع مرشحي كل الفهارس عبر كل مفاتيح الماركة، ويزيل تكرار
    (منافس، اسم) — الصف الواحد قد يصل من مفتاحين (المزدوج والمفرد)."""
    out: list = []
    seen: set = set()
    for bk in (brand_keys or [""]):
        for idx_obj in indices.values():
            for c in idx_obj.search(our_n, bk, size, ptype, gender,
                                    our_pline=our_pline, top_n=top_n,
                                    our_price=our_price):
                key = (c.get("competitor", ""), c.get("name", ""))
                if key not in seen:
                    seen.add(key)
                    out.append(c)
    return out
...
def match_single_product(
    ...
    # v35: جسر مفاتيح الماركة — الاسم قد لا يحمل الماركة وعمود سلة قد لا يُستخرَج
    brand_keys = _our_brand_candidates(brand, display_brand)
    our_pl = extract_product_line(product, _brand_for_pline(product, brand, brand_keys))

    # Gather candidates from all competitor indices (كل مفاتيح الماركة، بلا تكرار)
    all_cands: list = _search_indices(
        indices, our_n, brand_keys, size, ptype, gender,
...
def run_full_analysis(our_df, comp_dfs, progress_callback=None, use_ai=True,
    ...
    _subphase_timings: dict[str, float] = {}
    ...
    _subphase_timings["brand_vocabulary"] = round(_perf_counter() - _stage_started, 4)
    ...
    for cname, cdf in comp_dfs.items():
        ...
        # ج3 خ2: عمود المعرض (عدة صور) — منفصل عن c_img (صورة واحدة)، عرض فقط.
        c_gallery = _fcol_optional(cdf, ["image_urls", "الصور"])
        indices[cname] = CompIndex(
            cdf, ccol, icol, cname, img_col=c_img, url_col=c_url, gallery_col=c_gallery,
        )
    ...
    # v31.7: معالجة AI متوازية فعلياً (كان الاستيراد موجوداً بلا استخدام).
    # كل دفعة _ai_batch (شبكة، I/O) تُرسَل لخيط خلفي فتُكمل حلقة المطابقة
    # الرئيسية (CPU-bound) فوراً بدل انتظار الشبكة. _drain_pending في نهاية
    # الحلقة يبني النتائج بترتيب الإرسال الأصلي حصراً — لا تغيير في ترتيب
    # results عن السلوك المتسلسل السابق.
    from concurrent.futures import ThreadPoolExecutor
    _ai_executor = ThreadPoolExecutor(max_workers=3)
    _in_flight = []  # [(future, items)] بترتيب الإرسال

    def _apply_batch(items, idxs):
        """يبني صفوف النتائج لعناصر دفعة AI واحدة (idxs=None ⇒ fallback fuzzy) — محمي من الأخطاء"""
        if idxs is None:
        ...
        for j, it in enumerate(items):
            try:
                ...
                if rr is not None:
                    ...
                    # v37: تناظر مع مسار ≥85 — لا نُسقط منتجنا صامتاً إن أعاد _row
                    #      قيمة None؛ نحفظه صفّاً مستبعداً مرئياً (يغلق التوازن).
                    audit_stats["dropped_none"] = audit_stats.get("dropped_none", 0) + 1
                    results.append(_excluded_match_row(
                        it.get("product", ""), it.get("our_price", 0.0), it.get("our_id", ""),
                        it.get("brand", ""), it.get("size", 0), it.get("ptype", ""), it.get("gender", ""),
                        our_img=it.get("our_img", ""), our_url=it.get("our_url", ""),
                        no=it.get("our_no", ""),
                        score=0.0, مصدر_المطابقة="row_returned_none",
                    ))
                ...
                # v37: احفظ منتجنا كصفّ مستبعد بدل إسقاطه صامتاً — الاعتماد على
                #      recover_uncovered_catalog وحده يفقد التوائم (يُزيل تكرار
                #      الأسماء)، فنغلق الثغرة عند المصدر داخل المحرّك.
                try:
                    audit_stats["dropped_flush_error"] = audit_stats.get("dropped_flush_error", 0) + 1
                    results.append(_excluded_match_row(
                        it.get("product", ""), it.get("our_price", 0.0), it.get("our_id", ""),
                        it.get("brand", ""), it.get("size", 0), it.get("ptype", ""), it.get("gender", ""),
                        our_img=it.get("our_img", ""), our_url=it.get("our_url", ""),
                        no=it.get("our_no", ""),
                        score=0.0, مصدر_المطابقة="flush_row_error",
                    ))
                except Exception:
                    pass
                continue
    ...
    def _flush():
        """يرسل الدُفعة المعلّقة الحالية لخيط خلفي فوراً (لا حجب) ويُفرغ pending."""
        if not pending:
            return
        items = list(pending)
        pending.clear()
        ...
        def _call():
            try:
                return _ai_batch(items)
            except Exception:
                return None
        _in_flight.append((_ai_executor.submit(_call), items))

    def _drain_pending():
        """ينتظر كل دُفعات AI المُرسَلة ويبني نتائجها بترتيب الإرسال الأصلي (لا تغيير في ترتيب results)."""
        for fut, items in _in_flight:
    ...
    def _cell_clean(r, col):
        if not col:
            return ""
        # ⚡ WORKS WITH BOTH dict and Series
        if hasattr(r, "index"):
    ...
    # ⚡ PERFORMANCE FIX: استبدال iterrows البطيء بـ to_dict('records')
    _stage_started = _perf_counter()
    for i, row in enumerate(our_df.to_dict('records')):
        ...
        #      ويحرسها داخل search() فلتر التصنيف (_NONPERF) + حارس المجموعة
        #      المتناظر: مجموعة↔مجموعة تُطابَق، ومجموعة↔عطر فردي تبقى مرفوضة.
        #      (كان استبعاداً أعمى يُسقط كل مجموعاتنا، منها ~97 لها توأم مطابق.)
        ...
        our_n   = normalize(product)
        # v35: جسر مفاتيح الماركة — extract_brand(الاسم) وحدها كانت تُسقط أي
        #      منتج لا يحمل ماركة معروفة في اسمه إلى «مستبعد (لا يوجد تطابق)»
        #      رغم امتلاء عمود «الماركة» في الكتالوج (3,606 حالة في لقطة 2026-07-11)
        brand_keys = _our_brand_candidates(brand, brand_from_row)
        our_pl  = extract_product_line(product, _brand_for_pline(product, brand, brand_keys))

        # ── جمع المرشحين من كل الفهارس (كل مفاتيح الماركة، بلا تكرار) ──
        all_cands = _search_indices(
            indices, our_n, brand_keys, size, ptype, gender,
    ...
    del pending
    gc.collect()
    _subphase_timings["result_materialization"] = round(_perf_counter() - _stage_started, 4)
    audit_stats["matching_subphases_sec"] = _subphase_timings
...
def _first_product_page_url_from_dict(row):
    """أول رابط http لا يبدو ملف صورة مباشر (صفحة منتج) — نسخة dict."""
    for c, v in row.items():
        if v is None or (isinstance(v, float) and pd.isna(v)):
            continue
        s = str(v).strip()
...
def _first_image_url_from_dict(row):
    """أول قيمة في الصف تبدو رابط صورة — نسخة dict."""
    for c, v in row.items():
        u = _extract_image_url_from_cell(v)
        if u:
            return u
    return ""
...
def find_missing_products(our_df, comp_dfs):
    ...
    ⚠️ RETIRED-BROKEN (وُسمت 2026-07-25 بجرد `grep` شامل) — **لا تُوصَل قبل إصلاح**:

    1) **بلا مستدعٍ إطلاقاً:** صفر استدعاء في الإنتاج **وصفر في `tests/`**. المسار
       الحيّ لقسم المفقودات هو `CompetitorIntelligence.find_missing_products`
       (`engines/competitor_intelligence.py:270`) عبر `bootstrap.py:259`. تشابه
       الاسمين هو ما جعل هذه تُحسب حيّة في توثيق سابق — إيجابية كاذبة.

    2) **عطب كامن مُثبَت تجريبياً:** الحلقة تمرّ على `cdf.to_dict('records')`
       (`:3452`) فالصفّ **قاموس**، بينما `_cell_str` (`:3428`) يفحص `col not in
       r.index` — خاصيّة `Series` لا `dict` ⇒ `AttributeError` عند أول صفّ
       **إن وُجد عمود رابط في جدول المنافسين** (وهو موجود في المسار الحقيقي:
       «رابط المنتج»). لم ينكشف قط لأن الدالّة لا تُشغَّل.
       الإصلاح إن أُحييت: `_cell_str` يقبل `Mapping` (`col not in r`) لا `.index`
       — وموضعان يستدعيانه (`:3466` و`:3560`).

    تُبقى موسومةً لا محذوفة (نمط `RETIRED`/`RESERVED-KEEP` في المشروع): فيها
    منطق فرز ونيّة تصميم قد يُستفاد منهما، وحذف 415 سطراً لا يشتري شيئاً.
    """
    our_col = _name_col_for_analysis(our_df)

    # ── بناء فهرس منتجاتنا الكامل ─────────────────────────────────────
    # ⚡ PERFORMANCE FIX: استبدال iterrows البطيء بـ list comprehension على to_dict
    our_records = our_df.to_dict('records')
    our_items = []
    ...
    for cname, cdf in comp_dfs.items():
        ...
        for row in cdf.to_dict('records'):
            ...
            if not bare_ck or len(bare_ck) < 3:
                continue
            comp_sku_raw = _norm_sku_barrier(str(row.get(icol, "")))
            ...
            _cp_price = _price_from_dict(row)
            ...
            entry = {
                "منتج_المنافس":  cp,
                "معرف_المنافس":  _norm_sku_barrier(str(row.get(icol, ""))),
                "سعر_المنافس":   _price_from_dict(row),
                "المنافس":       cname,
                "الماركة":       c_brand,
...
# ⚰️ RETIRED-KEEP (2026-07-10 — بقرار المالك: وسمٌ قابل للتراجع، لا حذف):
# `smart_missing_barrier` حاجز مفقودات قديم — صفر مستدعين في المستودع كله حتى
# الاختبارات (تحقُّق grep 2026-07-10؛ الذكر الآخر الوحيد docstring في ownership_matcher).
# قِيس ضارّاً: يستبعد 1,334 صفاً ~91% منها مطابقات خاطئة، فحُسم إبقاؤه غير موصول عمداً.
# البديل الحيّ: services/ownership_matcher. لا يُستدعى لعمل جديد ولا يُحذف (منطق مطابقة —
# الحذر الأقصى). التراجع = حذف هذا التعليق فقط. لم يُمسّ جسم الدالّة.
def smart_missing_barrier(missing_df: pd.DataFrame, our_df: pd.DataFrame, threshold: int = 92) -> pd.DataFrame:
    ...
    for idx, row in enumerate(filtered_df.to_dict('records')):
        ...
        keep_idx.append(filtered_df.index[idx])
