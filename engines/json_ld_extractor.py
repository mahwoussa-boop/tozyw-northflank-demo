...
def _norm_availability(raw: Any) -> str:
    """يطبّع حالة المخزون من schema.org إلى رمز نظيف.

    ``https://schema.org/InStock`` → ``InStock``؛ ``OutOfStock``/``SoldOut`` تبقى
    كما هي (آخر مقطع بعد ``/``). فارغ ⇒ '' (لا افتراض — المُستهلك يقرّر).
    """
    if not raw:
        return ""
...
def _extract_from_meta(html: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "availability": _norm_availability(
            ...
            or meta.get("og:availability")
            or meta.get("product:stock_availability")
        ),
        "currency": cur or "SAR",
        "source":   "og-meta",
    }
