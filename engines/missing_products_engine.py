...
try:
    from engines.ai_engine import (
        _call_gemini as _ai_gemini,
        _call_openrouter as _ai_openrouter,
        _call_cohere as _ai_cohere,
    )

    def _call_gemini(prompt, system="", grounding=False, temperature=0.3, max_tokens=8192):
        """تدرّج موحّد Gemini → OpenRouter → Cohere (يعمل بمفتاح OpenRouter وحده)."""
        return (
            _ai_gemini(prompt, system=system, grounding=grounding,
                       temperature=temperature, max_tokens=max_tokens)
            or _ai_openrouter(prompt, system)
            or _ai_cohere(prompt, system)
        )
except Exception:
    _call_gemini = None
...
def detect_missing(comp_df: pd.DataFrame, catalog: pd.DataFrame,
    ...
    for original_idx, row in enumerate(comp_df.to_dict('records')):
        idx = comp_df.index[original_idx]
        q = row.get("_norm", "")
        if not q:
        ...
        if UNCERTAIN_LOWER <= score < EXISTS_THRESHOLD:
            ...
            continue
        # مفقود مؤكد
        k = _hash_key(row.get("اسم_المنتج", ""))
        if k in seen_keys:
            ...
            # invariant counts it.
            if float(row.get("السعر", 0) or 0) > seen_keys[k].price:
                seen_keys[k].price = float(row.get("السعر", 0) or 0)
                seen_keys[k].competitor = row.get("المنافس", "")
                seen_keys[k].url = row.get("الرابط", "")
            _led.mark_state(cid, CONFIRMED_MISSING,
                            reason_code="duplicate_missing",
                            last_score=float(score))
            continue
        seen_keys[k] = MissingProduct(
            name=row.get("اسم_المنتج", ""), price=float(row.get("السعر", 0) or 0),
            competitor=row.get("المنافس", ""), url=row.get("الرابط", ""),
            image=row.get("الصورة", ""), key=k, confidence="sure_missing",
        )
        _led.mark_state(cid, CONFIRMED_MISSING,
    ...
    if use_ai and uncertain and _call_gemini:
        for idx, row, score in uncertain:
            ...
            if _ai_is_missing(row.get("اسم_المنتج", ""), catalog, score):
                k = _hash_key(row.get("اسم_المنتج", ""))
                if k not in seen_keys:
                    seen_keys[k] = MissingProduct(
                        name=row.get("اسم_المنتج", ""), price=float(row.get("السعر", 0) or 0),
                        competitor=row.get("المنافس", ""), url=row.get("الرابط", ""),
                        image=row.get("الصورة", ""), key=k, confidence="ai_verified",
                    )
                if cid:
                    _led.mark_state(cid, CONFIRMED_MISSING,
        ...
        for idx, row, score in uncertain:
            ...
            if k not in seen_keys:
                seen_keys[k] = MissingProduct(
                    name=row.get("اسم_المنتج", ""), price=float(row.get("السعر", 0) or 0),
                    competitor=row.get("المنافس", ""), url=row.get("الرابط", ""),
                    image=row.get("الصورة", ""), key=k, confidence="uncertain",
                )
            if cid:
