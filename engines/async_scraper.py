...
def _gather_image_urls(main: str, raw) -> list:
    """يجمع روابط صور المنتج بالترتيب: الرئيسية أولاً ثم البقية، بلا تكرار.
    يقبل قائمة نصوص، أو قائمة dicts {"src":..}/{"url":..}، أو نصاً مفرداً.
    التنقية النهائية (روابط مباشرة .jpg/.png/.webp/.gif) تتم في db_manager._norm_image_urls."""
...
def extract_product(data: dict, store_url: str) -> dict | None:
    ...
    # كل روابط الصور (الرئيسية أولاً، بلا تكرار) — تُخزَّن كمصفوفة JSON في القاعدة.
    image_urls = _gather_image_urls(
        image, data.get("image_urls") or data.get("images") or [],
    )
                ...
                if resp is not None:
                    try:
                        if resp.status == 200 and "json" in resp.headers.get("Content-Type", ""):
                            ...
                            prod = data.get("product", data) if isinstance(data, dict) else data
                ...
                row = extract_product(
                    {
                        ...
                        "url":   url,
                        "brand": "",
                        # حالة المخزون الحقيقية من JSON-LD (بدل الافتراض «true»)
                        # — تغذّي خطّاف الحرجية (نفاد المخزون = فرصة حرجة).
                        "available": _ld_data.get("availability", "") or "",
                    },
                    store_url,
                )
                if _has_valid_price(row):
...
async def scrape_one_store(
    ...
    try:
        # 🟡 RESERVED-KEEP (2026-07-10 — بقرار المالك: توثيق قابل للتراجع، لا حذف):
        # وحدة engines/sitemap_resolve مفقودة من حادثة فقدان الملفات (2026-06-28)
        # ولم تُستَرجَع بعد من محجر مضاد الفيروسات. هذا المسار (async_scraper) خامل:
        # لا مستورد إنتاجي له (المسار الحيّ عبر services/scraper_service)، والاستيراد
        # أدناه مطوَّق بـ except ImportError فلا ينهار — الميزة صامتة فقط لا معطوبة.
        # «تشغيله» = استرجاع الوحدة المفقودة (مشروع مستقل مسجَّل في ذاكرة الحادثة). لا يُحذف.
            ...
            try:
                ...
                while True:
                    ...
                    for prod in prods:
                        ...
                        img = str(images[0].get("src", "")) if images and isinstance(images[0], dict) else (str(images[0]) if images else "")
                        # كل صور المنتج من Shopify (بترتيب العرض) — الرئيسية أولاً.
                        image_urls = [
                            str(i.get("src", "")) if isinstance(i, dict) else str(i) for i in images if (isinstance(i, dict) and i.get("src")) or (isinstance(i, str) and i)
                        ]
                        row = extract_product(
                            {"name": name, "price": price, "url": prod_url,
                             "image": img, "image_urls": image_urls,
