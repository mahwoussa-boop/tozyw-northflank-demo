...
from utils.data_paths import get_data_db_path  # noqa: E402 — بعد تهيئة اللوغر
...
class CompetitorIntelligence:
    ...
    def find_missing_products(self, our_catalog_df, page=0, per_page=25, filters=None) -> Tuple[List[Dict], int]:
        ...
        # ⚡ PERFORMANCE FIX: استبدال iterrows البطيء بـ vectorized ops
        ...
        names = our_catalog_df[name_col].fillna("").astype(str).tolist()
        brands = (
            our_catalog_df[brand_col].fillna("").astype(str).tolist()
            if brand_col else [""] * len(names)
        )
        ...
        try:
            conn = self._get_conn()
            cur = conn.execute(
                "SELECT product_name, brand, category, image_url, price, competitor, image_urls, "
                "COALESCE(first_seen_at, added_at, '') "
                f"FROM competitor_products_store {where} "
                f"{'AND' if where else 'WHERE'} price > 0 "
                "ORDER BY price DESC, COALESCE(first_seen_at, added_at, '') DESC, "
                "LOWER(COALESCE(brand, '')) ASC, LOWER(COALESCE(product_name, '')) ASC, "
                "LOWER(COALESCE(competitor, '')) ASC",
                params
            )
        except Exception as e:
        ...
        for row in _stream_rows():
            ...
            if fp not in missing_map:
                ...
                missing_map[fp] = {
                    ...
                    "image_urls": r.get("image_urls", ""),   # JSON: كل روابط الصور
                    # أقدم رصد عبر كل المنافسين — «متى ظهر أول مرة في السوق».
                ...
                if price > m["max_price"]:
                    m["max_price"] = price
                # أقدم رصد: نحتفظ بالأصغر (الأقدم) غير الفارغ — تواريخ ISO تقارَن نصياً.
                _fs_new = r.get("first_seen_at", "")
                _fs_cur = m.get("first_seen_at", "")
                if _fs_new and (not _fs_cur or _fs_new < _fs_cur):
                    m["first_seen_at"] = _fs_new
                ...
                # احتفظ بأغنى مجموعة صور: فضّل صفّاً يحمل مصفوفة فعلية على الفارغ.
                if not m.get("image_urls") and r.get("image_urls"):
                    m["image_urls"] = r["image_urls"]
                m["suggested_price"] = max(0, m["min_price"] - 1)

        # يُقرأ من الخارج ليصل القصّ إلى **واجهة المالك** لا إلى سجلّ لا يفتحه.
        # (تشخيص 2026-07-26: الحارس امتلأ عند 250,000 وقصّ ~96,000 منتجاً بصمت
        # لأشهر — فاختفت الشريحة الغالية كلها وبقي أعلى سعر 402.5 ر.س.)
        self.last_capped = _capped
        self.last_cap_limit = _MAX_UNIQUE
        self.last_cap_skipped_rows = _skipped_rows
        if _capped:
            log.warning(
                "find_missing: بلغ سقف المفقودات الفريدة %d (MISSING_MAX_UNIQUE) — "
                "أُوقف إنشاء عناصر فريدة جديدة لحماية الذاكرة. السقف 250000 "
                "معتمد؛ لا يُرفع قبل قياس الذاكرة في تشغيل طبيعي.", _MAX_UNIQUE,
            )
    ...
    def prepare_for_make(self, product: dict) -> dict:
        ...
        return {
            ...
            "رمز المنتج sku": "",
            "صورة المنتج": image,
            "صور المنتج": product.get("image_urls", ""),   # JSON: كل روابط الصور
