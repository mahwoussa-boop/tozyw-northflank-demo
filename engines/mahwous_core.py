...
def apply_strict_pipeline_filters(
    ...
    for idx, row in enumerate(df.to_dict('records')):
        ...
        if not name or name.lower() in ("nan", "none", "<na>"):
            ...
            excluded_reasons[original_idx] = "اسم فارغ"
            continue
        ...
        if _is_sample_strict(name):
            ...
            excluded_reasons[original_idx] = "كلمة عينة محظورة"
            continue
        ...
        if _classify_rejected(name):
            ...
            excluded_reasons[original_idx] = "تصنيف مستبعد (عينة/تقسيم)"
            continue
        ...
        if 0 < ml < min_ml:
            ...
            excluded_reasons[original_idx] = f"حجم صغير جداً ({ml} مل)"
            continue

        keep_idx.append(original_idx)
...
def validate_export_product_dataframe(df: pd.DataFrame) -> Tuple[bool, List[str]]:
    ...
    for i, row in enumerate(df.to_dict('records')):
        ...
        price = _safe_float(
            row.get("سعر_المنافس", row.get("سعر المنتج", row.get("السعر", 0)))
        )
