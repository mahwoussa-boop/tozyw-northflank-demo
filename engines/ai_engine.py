...
def diagnose_ai_providers() -> dict:
    ...
    for i, key in enumerate(GEMINI_API_KEYS or []):
        ...
        try:
            ...
            r = requests.post(_GU, json=payload, headers={"x-goog-api-key": key}, timeout=15)
    ...
    if OPENROUTER_API_KEY:
        try:
            r = requests.post(_OR, json={
                "model": "google/gemini-2.5-flash",  # ← مُتحقَّق (2.0-flash لم يعد صالحاً)
...
def _call_gemini(prompt, system="", grounding=False, temperature=0.3, max_tokens=8192):
    ...
    for i, key in enumerate(GEMINI_API_KEYS):
        if not key:
            continue
        try:
            r = _GEMINI_SESSION.post(_GU, json=payload, headers={"x-goog-api-key": key}, timeout=(5, 45))
            if r.status_code == 200:
                data = r.json()
...
def _call_openrouter(prompt, system=""):
    ...
    # ملاحظة: المعرّفات أدناه مُتحقَّقة فعلياً على OpenRouter (2026-06-28).
    # أُزيلت معرّفات قديمة: anthropic/claude-sonnet-4-20250514 (400 invalid)،
    # و *:free (لم تعد مجانية → 404).
    SMART_MODELS = [
        # ── المستوى 1: سريع + رخيص + ممتاز (الاستخدام اليومي) ──
        "google/gemini-2.5-flash",                    # ✅ أسرع + أذكى بالعربي (الأساسي)
        "anthropic/claude-sonnet-4",                  # ✅ دقة عالية جداً + JSON ممتاز
        # ── المستوى 2: احتياطي ذكي (إذا فشل المستوى 1) ──
        "deepseek/deepseek-chat-v3-0324",             # ✅ منطق قوي + رخيص
        "qwen/qwen-2.5-72b-instruct",                 # ✅ ممتاز بالعربي
        # ── المستوى 3: احتياطي أخير ──
        "meta-llama/llama-3.3-70b-instruct",          # ✅ احتياطي مستقر
    ]
    ...
    for model in SMART_MODELS:
        try:
            ...
            if r.status_code == 200:
                ...
                # حدّ المفتاح مُستنفَد (Key limit exceeded) — مستوى الحساب لا النموذج،
                # فإعادة المحاولة عبر بقية النماذج بلا جدوى. فشل سريع كـ 401.
                try:
                    msg = r.json().get("error", {}).get("message", "")[:120]
                except Exception:
                    msg = r.text[:120]
                _log_err("OpenRouter", f"حدّ المفتاح مُستنفَد (403) — {msg}")
                return None
            else:
                try:
                    msg = r.json().get("error", {}).get("message", "")
...
def gemini_chat(message, history=None, system_extra=""):
    ...
    for key in GEMINI_API_KEYS:
        if not key: continue
        try:
            r = requests.post(_GU, json=payload, headers={"x-goog-api-key": key}, timeout=40)
            if r.status_code == 200:
                data = r.json()
                if data.get("candidates"):
                    parts = data["candidates"][0]["content"]["parts"]
...
{{
  ...
  "fragrantica_url": "رابط الصفحة"
    ...
    grounded = bool(txt)                       # نجح البحث المؤرَّض (Grounding)؟
    if not txt:
        # احتياطي 1 بلا تأريض: النوتات من ذاكرة النموذج ⇒ ثقة منخفضة (قد تُلفَّق).
        # نُبقيه لئلا يبقى المنتج بلا أي بيانات، لكن نَسِمه `_grounded=False`
        # ليُعلّمه الحارس للمراجعة بدل نشره صامتاً.
        txt = _call_gemini(prompt)
    if not txt:
        # احتياطي 2: OpenRouter (مفتاح متوفّر) — يرفع تغطية النفحات للعطور النيش
        # التي لا يجدها تأريض Gemini. كذلك ذاكرة نموذج ⇒ يبقى `_grounded=False`
        # فيحجزه حارس البوابة (assess_notes_quality) للمراجعة قبل النشر (لا اختلاق صامت).
        txt = _call_openrouter(prompt)
    if not txt: return {"success":False}

    data = _parse_json(txt)
    if data: return {"success":True, "_grounded": grounded, **data}
    return {"success":False, "_grounded": grounded, "description_ar":txt[:200] if txt else ""}
...
def auto_infer_category(product_name: str, gender_hint: str = "") -> str:
    ...
    return "العطور > عطور للجنسين"  # مجهول الجنس ⇒ محايد لا «رجالية» مفروضة
