...
def _default_ai_cache_db() -> str:
    """مسار قاعدة كاش AI الافتراضي — دالة مستقلة كي تُعزل في الاختبارات (conftest)."""
    from utils.data_paths import get_data_db_path
    return get_data_db_path("pricing_v18.db")


class SqliteAICache:
    """كاش AI مؤرَّض عبر جدول ``ai_cache`` القائم (TTL ٩٠ يوماً — وحدة ج2).

    **حوكمة صارمة:** ``set`` بلا ``source`` صريح (اسم مزوّد حقيقي) لا يُكتب أبداً —
    ممنوع تخزين أي نص AI حرّ بلا أصل. القراءة تتجاهل القيود الأقدم من ``ttl_days``
    (تُعامَل كغياب كاش ⇒ المستدعي يعيد النداء ويكتب فوقها ``INSERT OR REPLACE``).
    كل عملية داخل try/except: فشل الكاش لا يُسقط استجابة AI حقيقية أبداً.
    """

    _DDL = """CREATE TABLE IF NOT EXISTS ai_cache (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        timestamp TEXT, prompt_hash TEXT UNIQUE,
        response TEXT, source TEXT
    )"""

    def __init__(self, db_path: Optional[str] = None, ttl_days: int = 90) -> None:
        self._db_path = db_path
        self._ttl_days = ttl_days

    def _resolve_db(self) -> str:
        return self._db_path or _default_ai_cache_db()

    def get(self, key: str) -> Optional[str]:
        import sqlite3
        from datetime import datetime, timedelta
        try:
            uri = "file:" + self._resolve_db().replace("\\", "/") + "?mode=ro"
            con = sqlite3.connect(uri, uri=True)
            try:
                row = con.execute(
                    "SELECT response, timestamp FROM ai_cache WHERE prompt_hash=?",
                    (key,),
                ).fetchone()
            finally:
                con.close()
            ...
            response, ts = row
            try:
                age_ok = datetime.now() - datetime.strptime(ts, "%Y-%m-%d %H:%M:%S") \
                    <= timedelta(days=self._ttl_days)
            except (TypeError, ValueError):
                return None  # طابع زمني فاسد ⇒ لا يُعامَل كإصابة
            return response if age_ok else None
        except Exception:
            return None  # القاعدة غائبة/الجدول غير موجود بعد ⇒ غياب كاش لا عطل

    def set(self, key: str, value: str, source: str = "") -> None:
        if not source:
            return  # حوكمة: صفر قيد بلا مصدر مؤرَّض
        import sqlite3
        from datetime import datetime
        try:
            conn = sqlite3.connect(self._resolve_db(), timeout=30)
            try:
                conn.execute(self._DDL)
                conn.execute(
                    "INSERT OR REPLACE INTO ai_cache (timestamp, prompt_hash, response, source) "
                    "VALUES (?,?,?,?)",
                    (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), key, value, source),
                )
                conn.commit()
            finally:
                conn.close()
        except Exception:
            pass  # فشل الكتابة لا يُسقط استجابة AI الحقيقية أبداً
        ...
        for provider in self._chain():
            for _ in range(self._max_retries):
                try:
                    ...
                    logger.debug("فشل مزوّد %s: %s", provider.name, exc)
                    response = None
                if response:
                    if use_cache:
                        self._cache.set(key, response, provider.name)
