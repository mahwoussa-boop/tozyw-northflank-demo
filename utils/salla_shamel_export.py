...
# ══════════════════════════════════════════════════════════════════════════
def _detect_product_type(product_name: str, category: str = "") -> str:
    """يكشف نوع المنتج: 'perfume' أو 'care' أو 'makeup'. خالص."""
    combined = f"{product_name} {category}".lower()
    _CARE_KW = (
        "شامبو", "كريم", "لوشن", "بلسم", "جل", "سيرم", "زيت شعر",
        "مرطب", "غسول", "صابون", "بادي ميست", "body mist", "deodorant",
        # «العناية» لا «عناية»: تصنيفات سلة للعناية دائماً «العناية بـ...»، بينما
        # البادئة العامة «جمال وعناية» تسبق العطور أيضاً — «عناية» المجرّدة كانت
        # تصنّف كل عطر «care» فيأخذ قالب العناية بدل العطور (إصلاح الجلسة 18).
        "ديودرنت", "بادي لوشن", "بادي كريم", "العناية", "hair", "body",
        "شاور جل", "shower", "lotion", "cream", "serum", "scrub",
        "mask", "ماسك", "مقشر",
    )
    _MAKEUP_KW = (
        "مكياج", "أحمر شفاه", "ماسكرا", "كحل", "بودرة", "فاونديشن",
        "كونسيلر", "آيلاينر", "بلاشر", "هايلايتر", "برونزر", "ظلال",
        "lipstick", "mascara", "foundation", "concealer", "eyeliner",
        "blush", "highlighter", "bronzer", "eyeshadow", "makeup",
    )
    if any(kw in combined for kw in _MAKEUP_KW):
        return "makeup"
    if any(kw in combined for kw in _CARE_KW):
        return "care"
    return "perfume"
...
def generate_salla_html_description(
    ...
    """
    قالب HTML ديناميكي — يفرّق بين العطور ومنتجات العناية/المكياج.

    الأقسام (عطور):
      1. h2 + p رئيسي
      2. h3 المكونات العطرية (الهرم)
      3. h3 لماذا تختار هذا العطر
      4. h3 الأسئلة الشائعة + رابط الماركة

    الأقسام (عناية/مكياج):
      1. h2 + p رئيسي + تفاصيل المنتج
      2. h3 المكونات الفعّالة
      3. h3 همسة من خبير مهووس
      4. h3 لماذا تختار + طريقة الاستخدام + أسئلة شائعة
    """
    ...
    # ── كشف نوع المنتج ──
    ptype = product_type or _detect_product_type(product_name, category)

    brand_link_html = (
        f'<p style="margin: 0;"><a href="{bu}" target="_blank" rel="noopener">اكتشف المزيد من منتجات {br}</a></p>'
        if brand_valid
        else '<p style="margin: 0;"><a href="https://mahwous.com/" target="_blank" rel="noopener">اكتشف المزيد من منتجات مهووس</a></p>'
    )

    # ══════════════════════════════════════════════════════════════════
    #  القالب الفرعي: عناية / مكياج
    # ══════════════════════════════════════════════════════════════════
    if ptype in ("care", "makeup"):
        type_label = "مستحضر تجميل" if ptype == "makeup" else "مستحضر عناية"
        fallback_desc = (
            f"{pn} {description_text or ''} {type_label} فاخر من {br}، "
            f"بحجم {size_label}، {gn}."
        )
        description_safe = _html_lib.escape(
            _safe_str(description_text) or fallback_desc, quote=False,
        )
        # المكونات الفعّالة تُطبَع فقط إذا حمل المنتج نص مكوّنات حقيقياً — وإلا تُسقَط
        # كلياً (لا نلفّق أسماء مكوّنات لا نعرفها). العطور لها قسمها الخاص (فرع else).
        _ing = _safe_str(ingredients)
        ingredients_html = (
            '<h3 style="margin: 12px 0 5px 0; color: #b8860b; font-size: 16px;">المكونات الفعّالة</h3>\n'
            '        <ul style="margin: 0 0 12px 0; padding-right: 20px;">\n'
            f'            <li style="margin-bottom: 4px;">{_html_lib.escape(_ing, quote=False)}</li>\n'
            '        </ul>'
        ) if _ing else ""
        raw_html = f"""
    <div dir="rtl" style="line-height: 1.5; font-family: Tahoma, Arial, sans-serif; color: #333;">
        ...
        <h3 style="margin: 12px 0 5px 0; color: #2c3e50; font-size: 16px;">تفاصيل المنتج</h3>
        <ul style="margin: 0 0 12px 0; padding-right: 20px;">
            <li style="margin-bottom: 4px;"><strong>الماركة:</strong> {br}</li>
            <li style="margin-bottom: 4px;"><strong>النوع:</strong> {type_label}</li>
            <li style="margin-bottom: 4px;"><strong>الحجم:</strong> {size_label}</li>
            <li style="margin-bottom: 4px;"><strong>الجنس:</strong> {gn}</li>
        </ul>

        {ingredients_html}

        <h3 style="margin: 12px 0 5px 0; color: #2c3e50; font-size: 16px;">همسة من خبير مهووس</h3>
        <p style="margin: 0 0 12px 0;">لأفضل نتيجة مع {pn}، استخدمه بانتظام واتبع تعليمات الاستخدام. المنتجات الأصلية من {br} تعطيك فرقاً واضحاً من أول أسبوع.</p>

        <p style="margin: 0 0 8px 0; font-weight: bold;">ضمان الأصالة 100% — مصادر موثوقة — تغليف هدية مجاني</p>

        <h3 style="margin: 12px 0 5px 0; color: #2c3e50; font-size: 16px;">لماذا تختار {pn}؟</h3>
        <ul style="margin: 0 0 12px 0; padding-right: 20px;">
            <li style="margin-bottom: 4px;">منتج أصلي 100% من {br}</li>
            <li style="margin-bottom: 4px;">{type_label} بجودة عالمية بسعر منافس</li>
            <li style="margin-bottom: 4px;">شحن سريع وتغليف هدية مجاني من مهووس</li>
        </ul>

        <h3 style="margin: 12px 0 5px 0; color: #2c3e50; font-size: 16px;">طريقة الاستخدام</h3>
        <ul style="margin: 0 0 12px 0; padding-right: 20px;">
            <li style="margin-bottom: 4px;">اتبع التعليمات المدوّنة على العبوة</li>
            <li style="margin-bottom: 4px;">استخدم الكمية المناسبة للحصول على أفضل نتيجة</li>
            <li style="margin-bottom: 4px;">للاستخدام الخارجي فقط</li>
        </ul>

        <h3 style="margin: 12px 0 5px 0; color: #2c3e50; font-size: 16px;">الأسئلة الشائعة</h3>
        <ul style="margin: 0 0 12px 0; padding-right: 20px;">
            <li style="margin-bottom: 4px;"><strong>هل المنتج أصلي؟</strong> نعم، جميع منتجاتنا أصلية 100% ومضمونة.</li>
            <li style="margin-bottom: 4px;"><strong>هل يناسب البشرة الحساسة؟</strong> تركيبته لطيفة ومناسبة لمعظم أنواع البشرة.</li>
        </ul>

        <p style="margin: 0 0 4px 0;">كلمات البحث: {pn.replace(' ', '، ')}</p>
        {brand_link_html}
    </div>
    """
    else:
...
def verify_truly_missing(
    ...
    # ── المطابقة بالبصمة (المسار الأساسي): يُستبعَد المؤكَّد (match) فقط، فلا
    #    يُخفى منتج جديد محتمل عن الملف. تدرّج للمنطق الاسمي القديم عند أي خطأ.
    try:
        from engines.duplicate_detector import CatalogIndex, is_duplicate, MATCH
        _idx = CatalogIndex.build(our_catalog_df.to_dict("records"))
        if len(_idx):
            _truly, _found = [], []
            for _, row in missing_df.iterrows():
                rd = row.to_dict()
                pname = _extract_product_name(rd)
                if not pname:
                    _truly.append(row)
                    continue
                verdict = is_duplicate({
                    "name": pname,
                    "brand": _extract_brand(rd),
                    "sku": _safe_str(rd.get("رمز المنتج sku") or rd.get("sku") or ""),
                    "barcode": _safe_str(rd.get("الباركود") or ""),
                }, _idx)
                (_found if verdict.decision == MATCH else _truly).append(row)
            return (
                pd.DataFrame(_truly).reset_index(drop=True) if _truly else pd.DataFrame(),
                pd.DataFrame(_found).reset_index(drop=True) if _found else pd.DataFrame(),
            )
    except Exception:
        pass  # تدرّج إلى المنطق الاسمي القديم أدناه
    ...
    for _, row in truly_missing.iterrows():
        ...
        if not description:
            description = generate_salla_html_description(
                ...
                ingredients=_safe_str(r.get("المكونات", "") or r.get("ingredients", "")),
            )
