...
def all_image_urls_list(s: str) -> list:
    """كل روابط الصور الصالحة في خلية واحدة (ج3 خ2: معرض بطاقة المقارنة) —
    نفس نمط ``first_image_url_string`` (فاصلة/عدة روابط في خلية واحدة) لكن
    يعيد القائمة كاملة بدل أول رابط فقط. لا يمسّ ``first_image_url_string``
    نفسها ولا مستخدميها."""
    s = (s or "").strip()
    if not s or ("http://" not in s and "https://" not in s):
        return []
    matches = [
        ...
        for m in _FIRST_HTTP_IMAGE_URL.finditer(s)
    ]
    if matches:
        return matches
    single = first_image_url_string(s)
    return [single] if single.startswith("http") else []
