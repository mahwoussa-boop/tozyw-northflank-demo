...
def ensure_indexes() -> None:
    ...
    _composite_index_specs = [
        ...
        # عدّاد/عرض «نفذت»: WHERE availability=0 ORDER BY updated_at DESC — الفهرس
        # المركّب يفلتر ويُرتّب من نفس الفهرس (لا مسح كامل 175k ولا فرز مؤقت).
        # idx_cps_avail الحالي (competitor, availability) لا يخدم «كل المنافسين».
        ("competitor_products_store", "idx_cps_avail_updated", "availability, updated_at DESC"),
        # هـ1 (قياس فتح الصفحات، 2026-07-22): شارة قِدَم البيانات (أ3 خ2) أضافت
        # GROUP BY product_url على 430k صف بلا فهرس في _data_ages()
        # (ui/components/comparison_card.py) — يحوّله لفرز/تجميع من الفهرس مباشرة.
        ("competitor_products_store", "idx_cps_url_updated", "product_url, updated_at"),
        # نفس الجلسة: latest_data_age_days() (opportunity_service) يحسب
        # MAX(updated_at) على القاعدة كلها — فهرس مفرد يتيح مسحاً عكسياً مبكر التوقف.
        ("competitor_products_store", "idx_cps_updated", "updated_at DESC"),
        # هـ1 (قياس فتح الصفحات، جلسة تالية): scraped_stats() يحسب
        # COUNT/MAX(updated_at) GROUP BY competitor على 430k صف — كان يستخدم
        # idx_cps_competitor2 (عمود مفرد) فيقرأ الصف الكامل من الجدول لكل تجميعة
        # (~3.8s)؛ فهرس مركّب مغطٍّ يحوّله لمسح فهرس صرف (~0.08s، تحقّق حي).
        ("competitor_products_store", "idx_cps_competitor_updated", "competitor, updated_at"),
    ]
    ...
    try:
        ...
        for idx in _redundant_indexes:
            try:
                ...
                pass
        # تسجيل هجرة فهرس عدّاد «نفذت» (idempotent — لا يفشل إن غاب db_version).
        try:
            c.execute(
                "INSERT OR IGNORE INTO db_version (version, description) VALUES "
                "('v31.9', 'idx_cps_avail_updated: فهرس عدّاد/عرض نفذت "
        ...
        try:
            c.execute(
                "INSERT OR IGNORE INTO db_version (version, description) VALUES "
                "('v31.10', 'idx_cps_url_updated + idx_cps_updated: فهرسا هـ1 "
                "لقياس/تسريع شارة قِدَم البيانات (_data_ages/latest_data_age_days)')"
            )
        except Exception:
            pass
        try:
            c.execute(
                "INSERT OR IGNORE INTO db_version (version, description) VALUES "
                "('v31.11', 'idx_cps_competitor_updated: فهرس هـ1 لتسريع "
                "scraped_stats() (كشط المنافسين) من ~3.8s إلى ~0.08s')"
            )
        except Exception:
            pass
        conn.commit()
...
# آلة حالة المنتج هذه نائمة بأكملها — جدولا `product_state` و`product_transitions`
# صفر صفوف فعلياً (قراءة ro مؤكَّدة)، والدوال السبع في هذه الكتلة (upsert_product_state،
# get_product_state، list_products_by_status، undo_last_transition، get_transitions،
# stale_products، state_health_counts) بلا أي مستدعٍ في المستودع كله — حتى في
# الاختبارات (تحقُّق grep شامل 2026-07-10).
# البدائل الحيّة العاملة: دفتر القرارات events (services/decision_journal ← log_event)
# للتدقيق، وسلة المهملات/hidden_products في اللقطة لمسار التراجع.
# لا تستدعِ هذه الكتلة لعمل جديد قبل قرار معماري. التراجع عن التقاعد = حذف هذا التعليق فقط.
...
# ─── قرارات ────────────────────────────────
# ⚰️ RETIRED-KEEP (2026-07-10 — بقرار المالك: وسمٌ قابل للتراجع، لا حذف):
# جدول `decisions` سجلٌّ قديم نائم — 0 صفوف فعلياً، ولا مستدعي حيّاً في المستودع
# لأيٍّ من log_decision/get_decisions أدناه (تحقُّق grep شامل 2026-07-10).
# البديلان الحيّان اللذان حلّا محلّه:
#   1) دفتر القرارات: services/decision_journal.record_decision → log_event (جدول events).
#   2) قرارات الذكاء: infrastructure/ai_decisions_repo.AIDecisionsRepo.log_decision
#      (دالّة مختلفة بنفس الاسم تكتب في جدول آخر — لا تخلط بينهما).
# لا تستدعِ الدالّتين أدناه لقرارات جديدة. التراجع عن التقاعد = حذف هذا التعليق فقط.
...
def append_price_decision(product_name, our_price, competitor="", price=0,
                          match_score=0, decision="", product_id=""):
    """
    يسجّل قرار تسعير **إلحاقيّاً** في price_history: سطرٌ جديد دائماً (INSERT صرف،
    بلا SELECT-ثم-UPDATE وبلا استبدال) — فتُحفظ رحلة القرار كاملةً بترتيبها الزمني.

    عمداً منفصلة عن ``upsert_price_history`` (التي تستبدل سعر اليوم في مكانه فتمحو
    تعدّد القرارات في اليوم الواحد). هذه تضيف كل قرار كحدثٍ مستقل لا يُمحى.

    - date: طابع زمني بدقّة الثانية ليتمايز أكثر من قرارٍ في اليوم نفسه.
    - our_price: السعر المعتمَد (المقترَح أو المعدَّل). price: مرجع السوق وقتها
      (وسيط عادةً). decision ∈ {"approved","edited"}. diff = our_price - price.
    تكتب في price_history فقط، تفتح get_db() وتعمل commit — لا تمسّ أي جدولٍ آخر.
    """
    conn = get_db()
    try:
        ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # دقّة الثانية (تمايز اليوم)
        try:
            diff = float(our_price) - float(price)
        ...
        conn.execute(
            """INSERT INTO price_history
               (date,product_name,competitor,price,our_price,diff,
                match_score,decision,product_id)
               VALUES (?,?,?,?,?,?,?,?,?)""",
            (ts, product_name, competitor, price, our_price, diff,
             match_score, decision, product_id),
        )
        conn.commit()
    finally:
...
# init_db_v26() runs FIRST so our_catalog/comp_catalog/processed_products exist
# before init_db() -> ensure_indexes() builds their indexes (e.g. idx_ourcat_norm)
# on a fresh DB. Without this, ensure_indexes() silently skips the missing tables
# and the app later crashes with "no such table: our_catalog".
init_db_v26()
init_db()
init_db_v35()
...
def get_our_catalog(limit: int = 100000):
    """يقرأ كتالوج متجرنا (معرّف + اسم) للمطابقة. آمن: try/finally يغلق الاتصال."""
    conn = get_db()
    try:
        rows = conn.execute(
            "SELECT product_id, product_name FROM our_catalog "
...
def migrate_db_v26():
    ...
    try:
        ...
        # ── 1. جدول سجل الأتمتة ──
        # 🟡 RESERVED-KEEP (2026-07-10 — بقرار المالك: وسمٌ توثيقي قابل للتراجع، لا حذف):
        # `automation_log` مخطَّطٌ محجوز لميزة «الأتمتة الذكية» المخفيّة عمداً
        # (HIDDEN_PAGES: «⚡ أتمتة Make» / «🔄 الأتمتة الذكية»). حالياً 0 صفوف وبلا أيّ
        # كاتب (INSERT) أو قارئ في المستودع كله (تحقُّق grep شامل 2026-07-10) — لا تفترض
        # أنه يلتقط شيئاً. جملة CREATE أدناه حيّة وضرورية (ترحيل idempotent) فتبقى كما هي؛
        # «تشغيله» = وصله بصفحات الأتمتة عند إظهارها لاحقاً. لا يُحذف.
...
def init_competitor_store() -> None:
    ...
    _new_cols = [
        ...
        ("image_urls",      "TEXT DEFAULT ''"),   # JSON: كل روابط صور المنتج (الرئيسية أولاً)
        ("rating_avg",      "REAL"),              # متوسط تقييم المنتج (all_rating.average)
    ]
...
def normalize_scraped_row_for_db(
    ...
    return {
        ...
        # كل روابط الصور (مصفوفة، الرئيسية أولاً) — تُسلسَل JSON عند الإدراج.
        "image_urls":  row.get("image_urls") or row.get("images") or [],
...
# ── تنقية وتسلسل روابط الصور المتعددة → JSON (للتخزين في competitor_products_store) ──
_IMG_EXTS_STORE = (".jpg", ".jpeg", ".png", ".webp", ".gif")


def _is_direct_image_url(u: str) -> bool:
    """رابط صورة مباشر (http/https + امتداد صورة) — يستبعد data:/placeholders."""
    s = str(u or "").strip()
    if not s.lower().startswith(("http://", "https://", "//")):
        return False
    low = s.lower().split("?", 1)[0]
    return any(ext in low for ext in _IMG_EXTS_STORE)


def _norm_image_urls(p: dict) -> str:
    """يرتّب ويُنقّي روابط صور المنتج → نص JSON: الرئيسية أولاً، روابط مباشرة فقط،
    بلا تكرار. يُعيد '' إن لم توجد روابط صالحة (فيُحافَظ على القديم عبر منطق COALESCE).
    يطابق ما يستهلكه utils.make_helper._collect_image_urls وحلقة Make."""
    import json as _json
    import re as _re2

    main = str(p.get("image_url", "") or "").strip()
    raw = p.get("image_urls") or p.get("images") or []
    ...
    for cand in [main, *raw]:
        ...
        u = str(cand or "").strip()
        if u and u not in seen and _is_direct_image_url(u):
            seen.add(u)
            out.append(u)
...
def upsert_competitor_products(
    ...
    try:
        ...
        # _existing_map[norm] = [id, image_url, product_url, brand, image_urls]  (قائمة لنحدّثها داخل الدفعة)
        _existing_map: dict = {}
        for _er in conn.execute(
            "SELECT id, norm_name, image_url, product_url, brand, image_urls "
            "FROM competitor_products_store WHERE competitor=?",
            (competitor,)
        ):
            _existing_map[_er["norm_name"]] = [
                _er["id"], _er["image_url"] or "", _er["product_url"] or "",
                _er["brand"] or "", _er["image_urls"] or ""
            ]
        ...
        for p in products:
            ...
            _imgs = _norm_image_urls(p)   # JSON: كل روابط الصور (الرئيسية أولاً)
            ...
            if _ex is not None:
                ...
                if _eid is not None:
                    ...
                    _old = _insert_params[_pi]
                    # تخطيط صف الإدراج (12 عموداً): [0]competitor [1]pname [2]norm [3]price
                    #   [4]image_url [5]product_url [6]brand [7]image_urls [8]size [9]gender
                    #   [10]added_at [11]updated_at — نُبقي الاسم/المقاس/الجنس/تاريخ الإضافة الأصلي.
                    _insert_params[_pi] = (
                        _old[0], _old[1], _old[2], price, _n_img, _n_url, _n_brand,
                        _n_imgs, _old[8], _old[9], _old[10], today
                    )
                # حدّث الخريطة كي يَسلسل أي تكرار لاحق فوق هذه القيم (مطابق للأصل).
                _existing_map[norm] = [_eid, _n_img, _n_url, _n_brand, _n_imgs]
                updated += 1
            else:
            ...
            conn.executemany(
                """INSERT INTO competitor_products_store
                   (competitor, product_name, norm_name, price, image_url,
                    product_url, brand, image_urls, size, gender, added_at, updated_at)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
                _insert_params
            )
        if _update_params:
            conn.executemany(
                """UPDATE competitor_products_store
                   SET price=?, updated_at=?, image_url=?, product_url=?, brand=?, image_urls=?
                   WHERE id=?""",
                _update_params
            )
...
# ── Bootstrap: تسجيل قائمة المنافسين القانونية (18 متجراً) عند أول تشغيل. ──
# نُقلت إلى نهاية الملف بعد تعريف الدالتين: كانت تُستدعى قبل تعريفهما (س.1615/1844)
# فتُلتقط بـ NameError وتُتخطّى بصمت ولا تُسجَّل المتاجر أبداً. idempotent (آمنة كل مرة).
try:
    migrate_db_v26()
