...
def _generate_mahwous_description(product: Dict) -> str:
    ...
    # 0) قالب مهووس الذهبي (الموحَّد) — الأولوية الأولى. حتمي بلا API.
    try:
        from engines.golden_template import build_golden_description  # type: ignore
        out = build_golden_description(product)
        if out and is_mahwous_description(out):
            return out
    except Exception as e:
        logger.debug("golden_template تعذّر: %s", e)
...
# قيم تعني «لا نوتة حقيقية» (هرم عطري مفقود أو حشو).
_NA_NOTE = {
    "", "غير متوفر", "غير متوفرة", "غير محدد", "غير معروف",
    "nan", "none", "[]", "n/a", "na", "-", "—",
}


def _is_perfume_category(cat: Any) -> bool:
    c = str(cat or "")
    return "عطور" in c or "العطور" in c


def assess_notes_quality(product: Dict) -> Tuple[bool, str]:
    """يقيّم جودة الهرم العطري لعطرٍ. يعيد (مراجعة_مطلوبة، سبب).

    سياسة سلامة بيانات متجر العطور: لا يُنشَر عطرٌ بنوتات مفقودة («غير متوفر»)
    أو غير مؤرَّضة (احتياطي AI بلا بحث ⇒ قابلة للتلفيق) **صامتاً** — يُعلَّم
    للمراجعة بدلاً من نشره. غير العطور (عناية/مكياج) لا تُفحَص.

    آمن ضد الإيجابيات الكاذبة: إن غابت حقول النوتات إطلاقاً (قد تكون ضمن
    الوصف) لا نحجب (fail-open). نحجب فقط بدليل موجب: حقول نوتات كلّها فارغة/
    «غير متوفر»، أو علم `_notes_grounded` صريحٌ بأنها غير مؤرَّضة.
    """
    getter = product.get if hasattr(product, "get") else (lambda k, d=None: d)
    cat = getter("اسم التصنيف") or getter("التصنيف_المنتج") or getter("category") or ""
    if not _is_perfume_category(cat):
        return False, ""
    fields = [
        getter("top_notes"), getter("مقدمة_العطر"), getter("الافتتاحية"),
        getter("heart_notes"), getter("middle_notes"), getter("قلب_العطر"),
        getter("base_notes"), getter("قاعدة_العطر"),
    ]
    present = [str(v).strip() for v in fields if v is not None and str(v).strip() != ""]
    if not present:
        return False, ""  # لا حقول نوتات ⇒ قد تكون ضمن الوصف؛ لا نحجب
    real = [v for v in present if v not in _NA_NOTE and v.lower() not in _NA_NOTE]
    if not real:
        return True, "⚠️ عطر بنوتات «غير متوفر» — مراجعة قبل النشر (هرم عطري مفقود)"
    if getter("_notes_grounded") is False:
        return True, "⚠️ نوتات غير مؤرَّضة (احتياطي AI بلا بحث) — مراجعة لتفادي تلفيق"
    return False, ""
...
def validate_and_enrich(
    ...
    *,
    ...
    for p in products or []:
        ...
        # ── جودة النوتات (عطور فقط) — لا تنشر هرماً مفقوداً/ملفّقاً صامتاً ──
        needs_review, review_reason = assess_notes_quality(p)
        if needs_review:
            logger.warning("⚠️ مراجعة «%s» — %s", name[:50], review_reason)
            rejected.append({"product": p, "reason": review_reason})
            continue
