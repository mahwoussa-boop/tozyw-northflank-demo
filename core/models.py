...
from core.enums import ActionType, ConfidenceLevel, Gender, SectionType
...
class AnalysisResult(BaseModel):
    ...
    duration_sec: float = 0.0  # مدّة التحليل الكامل بالثواني (للعرض في الملخّص)
...
# ════════════════════════════════════════════════════════════════════
#  قرار التسعير من الذكاء الاصطناعي — بحارس صارم ضد الهلوسة
# ════════════════════════════════════════════════════════════════════
class AIPriceDecision(BaseModel):
    """سعر مقترَح من الذكاء الاصطناعي، مُكبَّل بثوابت عمل لا يمكن خرقها.

    فلسفة منع الهلوسة: لا نثق بخرج النموذج أبداً، بل نفرض القواعد في الكود.
    الثوابت الصارمة (مضمونة دائماً بعد البناء):
      1) السعر المقترح رقمٌ منتهٍ موجب (لا صفر/سالب/NaN/∞) — وإلا يُرفض الصف.
      2) لا ينخفض أبداً عن «أرضية الربح» = التكلفة + الهامش الأدنى
         (مبلغاً ثابتاً أو نسبةً من التكلفة، أيّهما أعلى).
      3) لا يتجاوز «سقفاً منطقياً» = ``max_factor`` × أكبر مرجع سعري متاح
         (سعرنا الحالي أو أرخص منافس) لمنع المبالغة الجامحة.

    عند مخالفة (2) أو (3) لا نُسقط القرار، بل نقصّ السعر للحدّ الآمن ونرفع
    ``was_corrected`` مع ``correction_note`` — فيبقى المسار مستمراً وشفافاً
    (يستطيع العدّاد/الواجهة إظهار «صحّح النظام اقتراح AI من X إلى Y»).
    الأرضية (الربح) تتقدّم دائماً على السقف إن تعارضا — حماية الهامش أولوية.
    """

    model_config = ConfigDict(populate_by_name=True, str_strip_whitespace=True)

    product_id: str = Field(alias="معرف_المنتج")
    current_price: float = Field(default=0.0, alias="السعر")
    cost_price: float = Field(default=0.0, alias="التكلفة")
    min_margin: float = Field(default=0.0, description="هامش ربح أدنى مطلق (ريال)")
    min_margin_pct: float = Field(default=0.0, description="هامش ربح أدنى نسبة % من التكلفة")
    competitor_min_price: Optional[float] = Field(default=None, alias="سعر_المنافس")
    max_factor: float = Field(default=2.5, gt=0, description="سقف = max_factor × أكبر مرجع")

    suggested_price: float = Field(alias="السعر_المقترح")
    action: Optional[ActionType] = Field(default=None, alias="القرار")
    confidence: float = Field(default=0.0, alias="ثقة_AI", description="0..100")
    reasoning: str = Field(default="", alias="السبب")

    # تُملأ تلقائياً عند تفعيل حارس الأرضية/السقف (لا تُمرَّر يدوياً).
    was_corrected: bool = Field(default=False)
    correction_note: str = Field(default="")

    @field_validator(
        "current_price", "cost_price", "min_margin", "min_margin_pct",
        "competitor_min_price", "max_factor", mode="before",
    )
    @classmethod
    ...
    @classmethod
    def _validate_positive_finite(cls, v: Any) -> float:
        """الحارس الأول: رقم منتهٍ موجب أو نرفض الصف (هلوسة بنيوية)."""
        f = _to_float(v)
        if not math.isfinite(f) or f <= 0:
            raise ValueError(
                f"السعر المقترح يجب أن يكون رقماً موجباً منتهياً، لا: {v!r}",
            )
        return f

    @property
    def price_floor(self) -> float:
        """أرضية الربح = التكلفة + الهامش الأدنى (الأعلى بين المبلغ والنسبة)."""
        by_amount = self.cost_price + self.min_margin
        by_pct = self.cost_price * (1.0 + self.min_margin_pct / 100.0)
        return round(max(by_amount, by_pct), 2)

    @property
    def price_ceiling(self) -> Optional[float]:
        """السقف المنطقي = ``max_factor`` × أكبر مرجع سعري (أو None إن غاب)."""
        refs = [
            p for p in (self.current_price, self.competitor_min_price)
            if p is not None and p > 0
        ]
        if not refs:
            return None
        return round(max(refs) * self.max_factor, 2)

    @property
    def expected_margin(self) -> float:
        """هامش الربح المتوقّع عند السعر النهائي = المقترح − التكلفة."""
        return round(self.suggested_price - self.cost_price, 2)

    @model_validator(mode="after")
    def _enforce_guardrails(self) -> "AIPriceDecision":
        """الحارس الثاني/الثالث: يقصّ السعر داخل [الأرضية، السقف] بشفافية."""
        floor = self.price_floor
        ceiling = self.price_ceiling
        final = self.suggested_price
        notes: list[str] = []

        if final < floor:
            notes.append(f"رُفع من {final:.2f} إلى أرضية الربح {floor:.2f}")
            final = floor
        # السقف يُطبَّق فقط إن لم يُنزِل السعر تحت الأرضية (الربح أولوية).
        if ceiling is not None and ceiling >= floor and final > ceiling:
            notes.append(f"خُفض من {final:.2f} إلى السقف المنطقي {ceiling:.2f}")
            final = ceiling

        if notes:
            self.suggested_price = round(final, 2)
            self.was_corrected = True
            self.correction_note = " · ".join(notes)
        return self


# اسم مألوف يطلبه الاستهلاك الخارجي؛ المنافس = منتج منافس واحد.
