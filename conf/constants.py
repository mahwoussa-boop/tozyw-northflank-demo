...
# مجلد البيانات (القاعدة + الكاشات): مصدر واحد للحقيقة = utils.data_paths
# (متغيّر البيئة ``DATA_DIR`` أولاً — على Railway = /data للـVolume — وإلا
#  <جذر المستودع>/data. بيت Desktop\data القديم تقاعد بتاريخ 2026-07-04.)
DATA_DIR: Final[Path] = Path(get_data_dir())
...
MISSING_CACHE_PATH: Final[Path] = DATA_DIR / "missing_cache.json"
# كاش التحليل السعري الكامل (مخرجات run_full_analysis الثقيلة) — يُحسب مرّة.
PRICING_CACHE_PATH: Final[Path] = DATA_DIR / "pricing_cache.json"
# مصدر واحد دائم لقائمة المنافسين؛ لا تسقط إلى نسخة ثابتة داخل المستودع.
# ينشئ runtime_data_bootstrap.py الملف تحت DATA_DIR قبل تشغيل الواجهة.
...
# توقيع الكاش: <النسخة>|catalog_len|db_size|cap
# رُفعت F4v2⇒F5v1 (2026-07-26): تغيّر منطق الإنتاج — التصفية صارت **قبل** الدمج
# (كانت «عيّنة 1مل» تفوز بالدمج ثم تُسقَط فتمحو عائلة المنتج كلها). بلا رفع
# النسخة كان الكاش القديم يُعاد صامتاً ويُبطل الإصلاح دون أن يظهر شيء.
MISSING_CACHE_VERSION: Final[str] = "F5v1"
PRICING_CACHE_VERSION: Final[str] = "P1v2"   # توقيع كاش التسعير: P1v2|catalog_len|db_size|use_ai — v2: جسر مفاتيح الماركة + مواءمة حارس الفلانكرز (إبطال نتائج v1 المستبعدة خطأً)
