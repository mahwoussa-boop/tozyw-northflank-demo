...
try:
    ...
    _load_dotenv(override=False)  # احتياطاً: أي .env في مجلّد العمل الحالي
except ImportError:
...
WEBHOOK_UPDATE_QUANTITY = _s("WEBHOOK_UPDATE_QUANTITY")  # سيناريو منفصل: UpdateProduct (الكمية)
...
# ══════════════════════════════════════════════════
#  الدائرة المقفلة v2.0 — مصفوفة القرار + كلمات التنظيف العميق
# ══════════════════════════════════════════════════
# مصفوفة القرار (decide): العتبة تتحرّك حسب الاتفاق الهيكلي بدل خط مسطّح،
# مبرَّرة ببيانات Stage B (مطابقات حقيقية بتشابه اسم منخفض ⇒ لا تُرمى مفقودة).
CLOSED_LOOP_HIGH         = 80   # ‏≥80 وبلا تعارض هيكلي → متطابق
CLOSED_LOOP_STRUCT_FLOOR = 70   # ‏70-79 مع اتفاق هيكلي كامل → متطابق
CLOSED_LOOP_REVIEW       = 60   # ‏60-79 → مراجعة/AI ؛ <60 مع إشارة هيكلية → مراجعة

# ضجيج تسويقي لا يُغيّر هوية المنتج — يُزال للمطابقة فقط (deep_clean).
# ملاحظة مهمة: REJECT_KEYWORDS (عينة/decant/مينياتشر) تُغيّر الهوية (منتج مختلف)
# ⇒ ليست ضجيجاً وتبقى في فلتر الاستبعاد، لا في التنظيف. TESTER ضجيج آمن نعيد استخدامه.
CLOSED_LOOP_NOISE_WORDS = TESTER_KEYWORDS + [
    # ادّعاءات الأصالة (ضجيج شائع في عروض المنافسين)
    "original", "اصلي", "أصلي", "اصلية", "أصلية", "اصليه",
    "authentic", "genuine",
    # كلمات ترويجية بحتة
    "offer", "عرض", "عروض", "خصم", "تخفيض", "sale", "discount", "deal",
    "new", "جديد", "جديده", "جديدة",
]
# استبعاد معماري مقصود: «limited / edition / اصدار محدود / exclusive / hot / best»
# لم تُدرَج كضجيج لأنها قد تكون جزءاً من اسم خط منتج حقيقي (مثل Givenchy «Hot
# Couture» أو إصدار محدود = SKU مختلف فعلاً). تُراجَع لاحقاً عبر قائمة أمان.
