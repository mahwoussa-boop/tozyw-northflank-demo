...
### 🖱️ لصاحب المتجر (Windows) — نقرة واحدة
انقر مرّتين على **`تشغيل-مهووس.bat`**:
- أوّل مرّة: يثبّت Python والمكتبات تلقائياً (يحتاج إنترنت — دقائق معدودة).
- بعدها: يفتح المتصفّح تلقائياً على `http://localhost:8502`.
- اترك النافذة السوداء مفتوحة أثناء الاستخدام. (التفاصيل الكاملة في `اقرأني.txt`.)

### 💻 للمطوّر — أمران من جذر المشروع
```bash
pip install -r requirements.txt
streamlit run app.py
```
> ⚠️ شغّل من **جذر المشروع** كي تُحَل `engines/` و`utils/` و`config.py` القديمة.

## 🧪 الاختبارات

```bash
pytest -q          # 698 اختباراً (وحدة + تكامل) — من جذر المشروع
```
...
## 🚀 قائمة فحص «جاهز للإطلاق» ✅

نتيجة بروتوكول التحصين ما قبل الإطلاق (`tactical-harden`):

| # | البند | الحالة |
|---|------|--------|
| 1 | **الصياغة:** صفر أخطاء `py_compile` على 170 ملفاً | ✅ |
| 2 | **الأخطاء القاتلة:** صفر `F821`/`E9` داخل النطاق (`ruff`) | ✅ |
| 3 | **الاستيراد:** كل وحدات النطاق الأساسية تُستورد نظيفةً | ✅ |
| 4 | **الأخطاء الصامتة:** فشل الكاش/المخطط/التنزيل أصبح مرئياً (logging/UI) | ✅ |
| 5 | **تجربة المستخدم:** لا انهيار على غياب البيانات + توجيه المبتدئ للوحة | ✅ |
| 6 | **الأسرار:** صفر مفاتيح مكشوفة · `.env`/`secrets.toml` مستبعدة من git | ✅ |
| 7 | **الاعتماديات:** `requirements.txt` كامل ومثبّت الإصدارات (`==`) | ✅ |
| 8 | **التشغيل:** نقرة واحدة (`تشغيل-مهووس.bat`) + أمر مطوّر موثّق | ✅ |
| 9 | **الاختبارات:** 698 اختباراً تنجح كلها (`pytest -q` — 13 ثانية) | ✅ |

> **قبل الإطلاق:** تأكّد أن `data/` يحوي قاعدة المنافسين، وأن المنفذ 8502 غير مشغول.
